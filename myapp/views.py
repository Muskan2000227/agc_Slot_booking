...
def get_live_slots(request):
    """Fetch slots where the current time is within the slot time range."""
    ...
    # Define time range (from 30 seconds before to 30 seconds after)
    start_range = current_time - timedelta(seconds=30)
    end_range = current_time + timedelta(seconds=30)

    print(f"Checking for slots between: {start_range} and {end_range}")  # Debugging log

    live_slots = Slot.objects.filter(start_time__gte=start_range, start_time__lte=end_range, is_booked=True)
